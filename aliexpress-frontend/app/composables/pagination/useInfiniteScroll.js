...
// import { useObserverCore } from '../observer/useObserverCore'
// import { usePagination } from './useBasePagination'
...
import { useFillViewport } from "~/app/composables/pagination/useFillViewport"

/**
 * Generic Infinite Scroll
 *
 * @param {Object} opts
 * @param {Function} opts.loadMore - async fetch next page
 * @param {Ref<boolean>} opts.hasNext - reactive flag for next page
 * @param {Ref<boolean>} [opts.isLoading] - reactive loading flag
 * @param {boolean} [opts.prefetch=true] - auto load on intersection
 */
export function useInfiniteScroll(opts) {
    if (!opts?.loadMore || !opts?.hasNext) {
        throw new Error("useInfiniteScroll requires loadMore + hasNext")
    }
    ...
    async function safeLoad() {
        if (destroyed || opts.isLoading?.value) return false
        await opts.loadMore()
        if (!opts.hasNext.value) {
            core.unobserve(sentinelRef.value)
        }
        return opts.hasNext.value
    }
    ...
    const { checkAndLoadUntilScrollable } = useFillViewport(safeLoad, opts.hasNext, {
        maxLoops: opts.maxFillLoops ?? 20,
        debug: !!opts.debug,
    })
